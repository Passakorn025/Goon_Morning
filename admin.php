...
$sub = isset($_GET['sub']) ? $_GET['sub'] : 'uni_info'; // เพิ่มตัวนี้ไว้เช็ค Section
// ==========================================
    ...
    <div class="max-w-4xl mx-auto">
        <?php if($sub == 'uni_info'): ?>
            <div class="bg-white p-10 rounded-3xl border-2 border-gray-100 shadow-sm">
                <form action="admin.php?mode=edu" method="POST" class="space-y-6">
                    ...
                    <input type="text" name="uni_name" value="<?= $uni_data['uni_name'] ?>" class="w-full p-4 border rounded-xl font-bold" placeholder="ชื่อมหาลัย">
                    <input type="text" name="uni_img" value="<?= $uni_data['uni_img'] ?>" class="w-full p-4 border rounded-xl" placeholder="URL รูป">
                    <button type="submit" class="w-full bg-black text-white py-4 rounded-xl font-black uppercase">Save University</button>
                </form>
            </div>
            ...
            <div class="bg-white rounded-3xl border-2 border-gray-100 shadow-sm overflow-hidden">
                <table class="w-full text-left font-bold">
                    <tbody class="divide-y">
                        <?php $facs = $conn->query("SELECT * FROM faculties WHERE uni_id = $uid");
                        while($f = $facs->fetch_assoc()): ?>
                            <tr class="hover:bg-gray-50"><td class="p-6"><?= $f['fac_name'] ?></td></tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            </div>
        ...
        <?php elseif($sub == 'maj_detail'): ?>
            ...
            <?php if($selected_fac > 0): ?>
                <div class="space-y-4">
                    <?php 
                    $majors = $conn->query("SELECT * FROM majors WHERE fac_id = $selected_fac");
                    while($m = $majors->fetch_assoc()): ?>
                        <div class="bg-white rounded-[2rem] border-2 border-gray-100 shadow-sm overflow-hidden">
                            <button onclick="toggle('form-m-<?= $m['id'] ?>')" class="w-full p-6 text-left font-black uppercase italic flex justify-between items-center hover:bg-gray-50">
                                <?= $m['major_name'] ?> <span class="text-[10px] text-gray-300">Expand to Edit</span>
                            </button>
                            
                            <div id="form-m-<?= $m['id'] ?>" class="hidden p-10 border-t-2 border-dashed bg-gray-50/20">
                                <form action="admin.php?mode=edu" method="POST" class="space-y-8">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <input type="text" name="major_name" value="<?= $m['major_name'] ?>" class="p-4 border-2 rounded-xl font-bold" placeholder="ชื่อสาขา">
                                        <input type="text" name="major_detail" value="<?= $m['major_description'] ?>" class="p-4 border-2 rounded-xl" placeholder="รายละเอียด">
                                    </div>

                                    <div class="bg-white p-6 rounded-2xl border border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">รอบ</label><input type="number" name="round_open" value="<?= $m['round_open'] ?>" class="w-full p-2 border rounded-lg"></div>
                                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">ที่นั่ง</label><input type="number" name="seats" value="<?= $m['seats'] ?>" class="w-full p-2 border rounded-lg"></div>
                                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">GPAX</label><input type="text" name="gpax_min" value="<?= $m['gpax_min'] ?>" class="w-full p-2 border rounded-lg"></div>
                                        <div><label class="text-[10px] font-bold text-gray-400 uppercase">แผนการเรียน</label><input type="text" name="plan_accept" value="<?= $m['plan_accept'] ?>" class="w-full p-2 border rounded-lg"></div>
                                    </div>

                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <input type="number" name="score_tgat" value="<?= $m['score_tgat'] ?>" placeholder="TGAT %" class="p-3 border-2 border-blue-50 rounded-xl font-bold text-blue-600">
                                        <input type="number" name="score_tpat3" value="<?= $m['score_tpat3'] ?>" placeholder="TPAT3 %" class="p-3 border-2 border-blue-50 rounded-xl font-bold text-blue-600">
                                        <input type="number" name="score_math1" value="<?= $m['score_alevel_math1'] ?>" placeholder="Math1 %" class="p-3 border-2 border-blue-50 rounded-xl font-bold text-blue-600">
                                        <input type="number" name="score_phy" value="<?= $m['score_alevel_phy'] ?>" placeholder="Phy %" class="p-3 border-2 border-blue-50 rounded-xl font-bold text-blue-600">
                                    </div>

                                    <div class="bg-black text-white p-6 rounded-3xl grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div><label class="text-[9px] text-gray-500 uppercase">Salary Start</label><input type="text" name="salary_start" value="<?= $m['career_salary'] ?>" class="w-full bg-zinc-900 border-zinc-700 p-2 rounded-lg text-white"></div>
                                        <div><label class="text-[9px] text-gray-500 uppercase">Job Rate</label><input type="text" name="job_rate" value="<?= $m['career_job_rate'] ?>" class="w-full bg-zinc-900 border-zinc-700 p-2 rounded-lg text-white"></div>
                                        <div><label class="text-[9px] text-gray-500 uppercase">Market Demand</label><input type="text" name="market_demand" value="<?= $m['career_demand'] ?>" class="w-full bg-zinc-900 border-zinc-700 p-2 rounded-lg text-white"></div>
                                    </div>

                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <textarea name="prep_math" placeholder="พื้นฐานคณิตที่ควรมี..." class="p-4 bg-gray-100 rounded-xl border-0 shadow-inner"><?= $m['prep_math'] ?></textarea>
                                        <textarea name="prep_prog" placeholder="พื้นฐานโปรแกรมที่ควรมี..." class="p-4 bg-gray-100 rounded-xl border-0 shadow-inner"><?= $m['prep_prog'] ?></textarea>
                                    </div>

                                    <button type="submit" class="w-full bg-[#B1081C] text-white py-5 rounded-2xl font-black text-lg shadow-xl uppercase">Update Major Information</button>
                                </form>
                            </div>
                        </div>
                    <?php endwhile; ?>
