...
// ค้นหาจากชื่อ Category (หรือเปลี่ยนเป็นตารางที่มึงต้องการ)
$sql = "SELECT * FROM categories WHERE cat_name LIKE '%$search%' LIMIT 10";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        echo '<a href="P2.php?cat='.$row['id'].'" class="block px-6 py-3 hover:bg-gray-100 text-gray-800 border-b border-gray-50 transition-colors">';
        echo '<span class="font-bold">#</span> ' . $row['cat_name'];
        echo '</a>';
    }
} else {
}
...
<body>
   ...
   <header class="w-full bg-white border-b border-gray-200 h-20 flex items-center px-10 justify-between sticky top-0 z-50">
    ...
    <input type="tex t" id="searchInput" placeholder="ค้นหา..." 
           class="w-72 px-4 py-1.5 rounded-full border text-sm bg-white focus:outline-none focus:border-[#B1081C]" 
           autocomplete="off">
    
    <div id="searchResult" class="absolute left-0 w-full bg-white mt-1 rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-[999] hidden">
        </div>
</div>
    </header>
    ...
    <div class="w-full md:w-1/2 flex flex-col justify-center pl-24 z-20">
        ...
        <h1 class="text-[50px] font-extrabold leading-[1.1] text-[#1a1a1a]">
          โครงการแนะแนวการศึกษาต่อและอาชีพ <br> 
            <span class="text-[#B1081C] relative">
                Further Education and Career Guidance Program
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let type = 'uni'; // Default หน้าอื่นหา มหาลัย

    if (path === 'Index.php' || path === 'index.php' || path === '') {
        type = 'all'; // ถ้าหน้าแรก หาหมด
    } else if (path.includes('P1')) {
        type = 'job'; // ถ้าหน้า P1 หาเฉพาะงาน
    }
    ...
    // คลิกข้างนอกแล้วให้ปิดกล่องผลลัพธ์
